	...
	export default {
		...
		methods: {
			async loadData(){
				...
				// 	}
				// }) 
				Api.methods.getFirstLevelList({}).then(function(res){
					_self.flist=res.data
					_self.slist=res.data
					res.data.forEach(item=>{
						Api.methods.getSecondaryLevelList({"parentId":item.id}).then(function(res){
							_self.tlist=_self.tlist.concat(res.data)
						})
					})
				})
			},
